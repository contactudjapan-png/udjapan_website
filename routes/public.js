...
router.post(`/event/:id/polls/:pollId/vote`, async (req, res, next) => {
  try {
    ...
    if (req.xhr || req.get('Accept') === 'application/json') return res.json({ ok: false, error: err.message || 'Failed to cast vote.' });
...
router.post(`/event/:id/volunteer`, async (req, res, next) => {
  try {
    const { name, email, phone, preferred_task, preferred_duration } = req.body;
    if (!name || !email) {
